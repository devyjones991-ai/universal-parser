...
class APIAnalyticsService:
    ...
    async def _flush_metrics_loop(self):
        ...
        while True:
            try:
                ...
                logger.error(f"Error in flush metrics loop: {e}")
    ...
    async def _flush_metrics(self):
        ...
        try:
            ...
            logger.info(f"Flushed API metrics, {len(self.metrics_cache)} cache keys remaining")
    ...
    async def get_usage_stats(
        ...
        try:
            ...
            user_counts = {}
            for metric in metrics:
                if metric.metric_type == APIMetricType.REQUEST_COUNT and metric.user_id:
    ...
    async def _get_hourly_requests(self, metrics: List[APIMetric]) -> List[Dict[str, Any]]:
    ...
    async def _get_daily_requests(self, metrics: List[APIMetric]) -> List[Dict[str, Any]]:
    ...
    async def cleanup_old_metrics(self, days: int = 30):
        ...
        try:
            ...
            logger.info(f"Cleaned up API metrics older than {days} days")
            return True
